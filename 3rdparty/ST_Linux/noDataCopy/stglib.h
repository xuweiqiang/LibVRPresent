...
STGLIB_API_CALL int			STRenderVideoBuffer(STHDC hDC, STHBITMAP buffer, unsigned long physAddr, int blockW, int blockH, bool waitVSync, int u_offset, int v_offset, int yStride, int uStride, int vStride, t_sgaColorFormat format);
...
STGLIB_API_CALL int STSetFlipDelay(STHDC hDC, int flipdelay);

STGLIB_API_CALL int STColorKeyAlpha(STHDC hDC, bool keyAndAlpha, sga_uint16 colorKey,  sga_uint16 alphaFactor); //For Overlay Support
  //keyAndAlpha: true : enable
  //colorKey: RGB565 in framebuffer
  //alphaFactor: 0~255
