...
#define PAL_HEIGHT_RESOLUTION				288
...
STGLIB_API_CALL int			STLock(STHDC hDC, STVIDEOINDEINTERLACING dwMethod, unsigned int *pFIndex, t_STWinParams winSurfParams);
...
STGLIB_API_CALL int			STOverlay(STHDC hDC,int x0, int y0, int x1, int y1, STHBITMAP pSrc0, STHBITMAP pSrc1,int nXSrc, int nYSrc, STROP dwRop);
...
STGLIB_API_CALL int			STSetLayerInfo(STHDC hDC, int x0, int y0, int width, int height, int alpha, bool colorKey, int keyValue, void *pLayerSurface);
