...
interface file_io
{
	virtual u_int64_t GetFileSize(void) PURE;
	virtual u_int64_t GetPosition(void) PURE;
	virtual void SetPosition(u_int64_t) PURE;
	virtual u_int32_t ReadBytes(u_int8_t*, u_int32_t) PURE;
	virtual void WriteBytes(u_int8_t*, u_int32_t) PURE;
};

MP4FileHandle MP4Link(
	const file_io* fileIO, 
	u_int32_t verbosity DEFAULT(0));
...
MP4SampleId MP4GetNextSyncSample(
	MP4FileHandle hFile,
	MP4TrackId trackId, 
	MP4SampleId sampleId);
