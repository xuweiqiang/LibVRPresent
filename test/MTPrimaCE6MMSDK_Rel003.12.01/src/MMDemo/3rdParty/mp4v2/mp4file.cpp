...
void MP4File::Link(const file_io* fileIO)
{
	m_file_io=const_cast<file_io*>(fileIO);
	m_fileSize = m_file_io?m_file_io->GetFileSize():0; 
...
#else
#ifdef _UNICODE
	TCHAR tempFileName[MAX_PATH + 3];
	GetTempFileName(_T("."), // dir. for temp. files 
					_T("mp4"),                // temp. filename prefix 
					0,                    // create unique name 
					tempFileName);        // buffer for name
	WideCharToMultiByte(CP_ACP, 0, tempFileName, _tcslen(tempFileName), m_tempFileName, 0, NULL, NULL);
#else
	GetTempFileName(_T("."), // dir. for temp. files 
					_T("mp4"),                // temp. filename prefix 
					0,                    // create unique name 
...
int MP4File::Peek(MP4TrackId track1, MP4SampleId sample1, MP4TrackId track2, MP4SampleId sample2)
{
	u_int32_t nNumStreams = GetNumberOfTracks();
	ASSERT(track1<=nNumStreams && track2<=nNumStreams);
	int nRet = MP4_INVALID_TRACK_ID;
	u_int64_t nOffset;

	MP4Track *pTrack = GetTrack(track1);
	if((MP4_INVALID_SAMPLE_ID!=sample1) && (sample1<pTrack->GetNumberOfSamples()))
	{
		nOffset = pTrack->GetSampleFileOffset(sample1);
		nRet = track1;
	}

	pTrack = GetTrack(track2);
	if((MP4_INVALID_SAMPLE_ID!=sample2) && (sample2<pTrack->GetNumberOfSamples()))
	{
		u_int64_t nOffset2 = pTrack->GetSampleFileOffset(sample2);
		if((nRet>0) && (nOffset2<nOffset))
		{
			nOffset = nOffset2;
			nRet = track2;
		}
...
MP4SampleId MP4File::GetNextSyncSample(MP4TrackId trackId, MP4SampleId sampleId)
{
	return m_pTracks[FindTrackIndex(trackId)]->
		GetNextSyncSample(sampleId);
}
