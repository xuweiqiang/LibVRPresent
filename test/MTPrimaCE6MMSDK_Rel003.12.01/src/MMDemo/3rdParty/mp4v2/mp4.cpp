...
extern "C" MP4FileHandle MP4Link(const file_io* fileIO, u_int32_t verbosity)
{
	MP4File* pFile = NULL;
	try {
		pFile = new MP4File(verbosity);
	...
	MP4SampleId sample2)
{
	if (MP4_IS_VALID_FILE_HANDLE(hFile)) {
		try {
			return ((MP4File*)hFile)->Peek(track1, sample1, track2, sample2);
		}
		catch (MP4Error* e) {
			PRINT_ERROR(e);
			delete e;
...
extern "C" MP4SampleId MP4GetNextSyncSample(
	MP4FileHandle hFile,
	MP4TrackId trackId, 
	MP4SampleId sampleId)
{
	if (MP4_IS_VALID_FILE_HANDLE(hFile)) {
		try {
			return ((MP4File*)hFile)->GetNextSyncSample(
				trackId, sampleId);
		}
		catch (MP4Error* e) {
			PRINT_ERROR(e);
