	...
	try {
		...
		if (addHeader) {
			...
			*pDest++ =
			...
			u_int16_t s = m_rtpSequenceStart + pPacket->GetSequenceNumber();
			*((u_int16_t*)pDest) = 	(s << 8)| (s >>8);
			pDest += 2; 

			u_int32_t l = m_rtpTimestampStart + (u_int32_t)m_readHintTimestamp;
			*((u_int32_t*)pDest) = 	(l << 24) | ((l & 0xFF00) << 8) | ((l & 0xFF0000 )>> 8) | (l>>24);
			pDest += 4; 

			*((u_int32_t*)pDest) = (ssrc << 24) | ((ssrc & 0xFF00) << 8) | ((ssrc & 0xFF0000 )>> 8) | (ssrc>>24);
				//htonl(ssrc);
			pDest += 4;
		}
