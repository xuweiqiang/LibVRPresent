...
static size_t avi_read(IFileOps* pFileSys, unsigned char *buf, size_t len)
{
   int n = 0;
   size_t r = 0;

   n = len;
   if(pFileSys->Read(buf, n) != FERR_NOERROR)
   {
	  return 0;
   }
   return n;
}
//return: bytes been written
static size_t avi_write (IFileOps* pFileSys, unsigned char *buf, size_t len)
{
   int n = 0;
   size_t r = 0;

   n = len;
   if(pFileSys->Write(buf, n) != FERR_NOERROR)
	  return 0;
   return n;
}
...
static int avi_add_chunk(avi_t *AVI, char *tag, unsigned char *data, int length)
{
   unsigned char c[8];
   ...
   if( avi_write(AVI->pFileSys, c, 8) != 8 ||
       avi_write(AVI->pFileSys, data, length) != (size_t)length )
   {
	   distance = AVI->pos;
	   AVI->pFileSys->Seek(distance, FILE_SEEK_BEGIN); //SetFilePointer(AVI->fdes, AVI->pos, NULL, FILE_BEGIN);
...
avi_t* AVI_open_output_file(IFileOps* pFileSys)
{
   ...
   AVI->pFileSys = pFileSys;
   if(AVI->pFileSys == NULL)
   {
   ...
   i = avi_write(AVI->pFileSys, AVI_header, HEADERBYTES);
   if (i != HEADERBYTES)
   {
      //fclose(AVI->fdes); don't close it here
      AVI_errno = AVI_ERR_WRITE;
      free(AVI);
      return 0;
   }
...
#define AVIF_COPYRIGHTED        0x00020000
   ...
   long distance = 0;
   if(AVI->pFileSys->Seek(distance, FILE_SEEK_BEGIN) != FERR_NOERROR ||
      avi_write(AVI->pFileSys, AVI_header, HEADERBYTES) != HEADERBYTES)
   {
   ...
   //truncate
   int err = AVI->pFileSys->Truncate(AVI->pos);
   assert(err == FERR_NOERROR);
   return 0;
}
...
//   nhb += MAX_INFO_STRLEN;
   ...
   distance = AVI->pos;
   if(AVI->pFileSys->Seek(distance, FILE_SEEK_BEGIN) == -1)
   {
	   AVI_errno = AVI_ERR_CLOSE;
	   return -1;
   }	 
...
int AVI_append_audio(avi_t *AVI, char *data, long bytes)
{
  ...
  //update chunk header
  long distance = pos+4;
  AVI->pFileSys->Seek(distance, FILE_SEEK_BEGIN);
  long2str(c, length+bytes);     
  avi_write(AVI->pFileSys, c, 4);

  distance = pos+8+length;
  AVI->pFileSys->Seek(distance, FILE_SEEK_BEGIN);

  i=PAD_EVEN(length + bytes);

  bytes = i - length;
  avi_write(AVI->pFileSys, (unsigned char*)data, bytes);
  AVI->pos = pos + 8 + i;
...
avi_t *AVI_open_input_file(IFileOps* pFileSys, int getIndex)
{
  ...
  if(AVI==NULL)
    {
	  RETAILMSG(1, (L"AVI_open_input_file: not enough memory \n"));
      AVI_errno = AVI_ERR_NO_MEM;
      return 0;
    }
  memset((void *)AVI,0,sizeof(avi_t));
  ...
  AVI->pFileSys = pFileSys;//_wfopen(filename, L"rb");
  if(AVI->pFileSys == NULL)
  {
	  RETAILMSG(1, (L"AVI_open_input_file: FileName Error \n"));
     AVI_errno = AVI_ERR_OPEN;
     free(AVI);
     return 0;
  }
//  if(AVI->pFileSys->Open(filename, false) != FERR_NOERROR)
//  {
//	 AVI_errno = AVI_ERR_OPEN;
//	 free(AVI);
//     return 0;
...
int avi_parse_input_file(avi_t *AVI, int getIndex)
{
  ...
  long dist = 0;
  AVI->pFileSys->Seek(dist, FILE_SEEK_BEGIN);

  if( avi_read(AVI->pFileSys, data, 12) != 12 ) ERR_EXIT(AVI_ERR_READ)

   if( _strnicmp((char*)data  ,"RIFF",4) !=0 ||
       _strnicmp((char*)data+8,"AVI ",4) !=0 ) ERR_EXIT(AVI_ERR_NO_AVI)
   ...
   while(1)
   {
      if(avi_read(AVI->pFileSys, data, 8) != 8 ) break; /* We assume it's EOF */
	  ...
	  if(_strnicmp((char*)data, "LIST", 4) == 0)
      {
         if( avi_read(AVI->pFileSys, data, 4) != 4 ) ERR_EXIT(AVI_ERR_READ)
         n -= 4;
         if(_strnicmp((char*)data, "hdrl", 4) == 0)
         {
            hdrl_len = n;
	     ...
	     // offset of header
	     distance = 0;
		 if(AVI->pFileSys->Seek(distance, FILE_SEEK_SET) != FERR_NOERROR)
		    distance = -1;
		 pos = distance; //SetFilePointer(AVI->fdes, 0, NULL, FILE_CURRENT);
		 
		 header_offset = (pos == -1)? 0: pos;
				 
         if( avi_read(AVI->pFileSys, hdrl_data, n) != (size_t)n ) ERR_EXIT(AVI_ERR_READ)
         }
         else if(_strnicmp((char*)data,"movi",4) == 0)
         {
			distance = 0;
			if(AVI->pFileSys->Seek(distance, FILE_SEEK_SET) != FERR_NOERROR)
			   distance = -1;
			pos = distance; //SetFilePointer(AVI->fdes, 0, NULL, FILE_CURRENT);
			AVI->movi_start = (pos==-1)? 0: pos;//fseek(AVI->fdes,0,SEEK_CUR);
            
			distance = n;
			AVI->pFileSys->Seek(distance, FILE_SEEK_SET);
         }
         else
		 {
			distance = n;
      ...
      else if(_strnicmp((char*)data,"idx1",4) == 0)
      {
         ...
         if(avi_read(AVI->pFileSys, (unsigned char *) AVI->idx, n) != (size_t)n ) ERR_EXIT(AVI_ERR_READ)
      }
      else
	  {
         distance = n;
		 AVI->pFileSys->Seek(distance, FILE_SEEK_SET);//SetFilePointer(AVI->, n, NULL, FILE_CURRENT);      
	  }
   }
   ...
   long distance = AVI->movi_start;
   AVI->pFileSys->Seek(distance, FILE_SEEK_BEGIN);//SetFilePointer(AVI->fdes, , NULL, FILE_BEGIN);
   /* get index if wanted */
   ...
   if(AVI->idx)
   {
      ...
      distance = pos;
	  AVI->pFileSys->Seek(distance, FILE_SEEK_BEGIN);//SetFilePointer(AVI->fdes, pos, NULL, FILE_BEGIN);
      if(avi_read(AVI->pFileSys, data, 8) != 8) ERR_EXIT(AVI_ERR_READ)
      if( _strnicmp((char*)data, (char*)(AVI->idx[i]), 4) == 0 && 
		  str2ulong(data+4) == (unsigned long)len)
      {
         idx_type = 1; /* Index from start of file */
      }
      else
      {
         distance = pos+AVI->movi_start-4;
		 AVI->pFileSys->Seek(distance, FILE_SEEK_BEGIN);//SetFilePointer(AVI->fdes, pos+AVI->movi_start-4, NULL, FILE_BEGIN);

         if(avi_read(AVI->pFileSys, data, 8)!=8) ERR_EXIT(AVI_ERR_READ)
         if( _strnicmp((char*)data, (char*)(AVI->idx[i]), 4)==0 && str2ulong(data+4)==(unsigned long)len )
         {
   ...
   if(idx_type == 0)
   {
      ...
      distance = AVI->movi_start;
	  AVI->pFileSys->Seek(distance, FILE_SEEK_BEGIN);//SetFilePointer(AVI->fdes, AVI->movi_start, NULL, FILE_BEGIN);
      
      AVI->n_idx = 0;

      while(1)
      {
         if(avi_read(AVI->pFileSys, data, 8) != 8 ) break;
         n = str2ulong(data+4);
         ...
         {
            distance = 4;
			AVI->pFileSys->Seek(distance, FILE_SEEK_SET);//SetFilePointer(AVI->fdes, 4, NULL, FILE_CURRENT);
		    continue;
         }
		      ...
		      (data[3]=='b' || data[3]=='B')))
		 {
	         distance = 0;
			 AVI->pFileSys->Seek(distance, FILE_SEEK_SET);
			 avi_add_index_entry(AVI, (char*)data, 0, 
				                 distance - 8, n);
         }
	     distance = PAD_EVEN(n);
		 AVI->pFileSys->Seek(distance, FILE_SEEK_SET);//SetFilePointer(AVI->fdes, PAD_EVEN(n), NULL, FILE_CURRENT);
       }
      idx_type = 1;
   }
...
long AVI_read_frame(avi_t *AVI, char *vidbuf, int *keyframe)
{
   ...
   *keyframe = (AVI->video_index[AVI->video_pos].key==0x10) ? 1:0;
   ...
   long distance = AVI->video_index[AVI->video_pos].pos;
   AVI->pFileSys->Seek(distance, FILE_SEEK_BEGIN);//SetFilePointer(AVI->fdes, AVI->video_index[AVI->video_pos].pos, NULL, FILE_BEGIN);
 
   if (avi_read(AVI->pFileSys, (unsigned char*)vidbuf, n) != (size_t)n)
   {
...
long AVI_read_audio(avi_t *AVI, char *audbuf, long bytes)
{
   ...
   while(bytes>0)
   {
      ...
      if(avi_read(AVI->pFileSys, (unsigned char*)(audbuf+nr), todo) != (size_t)todo)
      {
         AVI_errno = AVI_ERR_READ;
         return -1;
      }
 ...
 */
   ...
   while(1)
   {
      ...
      if( avi_read(AVI->pFileSys, (unsigned char*)data, 8) != 8 ) return 0;
      ...
      if(_strnicmp(data,AVI->video_tag,3) == 0)
      {
         *len = n;
         ...
         {
            distance = n;
			AVI->pFileSys->Seek(distance, FILE_SEEK_SET);//SetFilePointer(AVI->fdes, n, NULL, FILE_CURRENT);
            return -1;
         }
         if(avi_read(AVI->pFileSys, (unsigned char*)vidbuf, n) != (size_t)n ) return 0;
         return 1;
      }
      else if(_strnicmp(data,AVI->track[AVI->aptr].audio_tag,4) == 0)
      {
         *len = n;
         ...
         if(avi_read(AVI->pFileSys, (unsigned char*)audbuf, n) != (size_t)n ) return 0;
         return 2;
         break;
      }
      else
	  {
         distance = n;
		 if(AVI->pFileSys->Seek(distance, FILE_SEEK_SET) != FERR_NOERROR)//SetFilePointer(AVI->fdes, n, NULL, FILE_CURRENT) == -1)  
			return 0;
   	  }	  
   }
}
...
char *AVI_strerror()
{
   ...
   if(AVI_errno == AVI_ERR_OPEN ||
      ...
      AVI_errno == AVI_ERR_CLOSE )
   {
      sprintf(error_string,"%s",avi_errors[aerrno]); // strerror(errno));
      return error_string;
   }
   else
   {
