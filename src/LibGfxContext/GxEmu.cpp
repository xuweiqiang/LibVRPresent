// GXdll.cpp : Defines the entry point for the DLL application.
//
...
//The way it works is that for 16 bit color
//parameter 3 must be 2 and parameter 4 must be 2*width
#define ALCUSTOM	        { 320, 240,   2, 640, 16, kfDirect | kfDirect565,                    TRUE }
#define CUSTOM              { 640, 480,   2, 1280, 16, kfDirect | kfDirect565,                   TRUE }
#define CUSTOM_SVGA         { 800, 600,   2, 1600, 16, kfDirect | kfDirect565,                   TRUE }
...
namespace
{
   ...
   //Config      l_config         = CASIO_E125; //SIMPLE_565;
	Config      l_config         = CUSTOM_SVGA;
   // Internal variables
...
int GXOpenDisplay( HWND hwnd, DWORD dwFlags )
{
   ...
   //MessageBox(NULL,TEXT("GX open"),TEXT("Status"),MB_OK);
   if (!::IsWindow(hwnd))
      return FALSE;
   ...
   if (dwFlags & GX_FULLSCREEN)   //Alex Sept 18      
		GXSetViewport( 0, l_config.height, 0, 0 ); //Alex Sept 18
...
void* GXBeginDraw()
{
   ...
   //MessageBox(NULL,TEXT("GX Begin draw"),TEXT("Status"),MB_OK);
	if (l_bSuspended)
	{
	  //MessageBox(NULL,TEXT("Cannot Begin draw"),TEXT("Error"),MB_OK);
      return 0;
	}
   return l_pVideoBits;
}
...
int GXEndDraw()
{
   ...
   //MessageBox(NULL,TEXT("GX End draw"),TEXT("Status"),MB_OK);
   if (!::IsWindow(l_hwnd))
      return FALSE;
...
*/
   if ( l_orientation == ORIENTATION_RIGHT )
   {
	  ...
	  // ::StretchBlt( hDestDC, 0, (l_viewportTop-75), l_config.width, l_config.height, 
		//   hSrcDC, l_config.width - 1, 0, -l_config.width, l_config.height, SRCCOPY );
	   // We use StretchBlt() to rotate the display
	   ::StretchBlt( hDestDC,
		   0, (l_viewportTop-75),
   ...
   {	  
      // A simple BitBlt() will do
	   //Special offset of 25 to comp for the WinCE menu bar
	   
#if 1
	   ::BitBlt( hDestDC,
		   0, (l_viewportTop /*-25*/),
		   l_config.width, l_viewportHeight,
		   hSrcDC,
		   0, l_viewportTop,
		   SRCCOPY );
#else
	   // this needs to be fixed next - achung
	   // the following below is more correct, and prevents a black bar on some systems.
...
int GXSetViewport( DWORD top, DWORD height, DWORD dwReserved1, DWORD dwReserved2 )
{
   ...
   {
      //MessageBox(NULL,TEXT("ERROR"),TEXT("CHECK VIEWPORT"),MB_OK);
      return FALSE;
   }
